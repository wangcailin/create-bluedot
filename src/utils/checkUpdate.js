...
async function checkUpdate(name, { version, packagePath } = {}) {
  ...
  function getLocalVersion(name) {
    ...
    packagePath =
      packagePath ||
      require.main.paths
        .concat(`${require(`path`).parse(process.execPath).dir}/node_modules`) // 全局安装目录
  ...
  return new Promise((resolve, reject) => {
    if (getLocalVersionRes !== getServerVersionRes) {
      console.log(
        `${name} 已发布新版本 ${getServerVersionRes}, 你当前版本为 ${getLocalVersionRes}`
      );
      console.log(`${chalk.red('更新执行：')}npm i -g create-bluedot@${getServerVersionRes}`);
      reject({});
    } else {
      resolve({});
    }
